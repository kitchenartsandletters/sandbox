class ItemsScroll extends HTMLElement {
  ...
  constructor() {
    ...
    this.scrollableElement = this.items;
  ...
  #getItemAtScroll() {
    ...
    const itemWidth = this.items.firstElementChild.getBoundingClientRect().width;
    const itemFullWidth = itemMarginLeft + itemWidth + itemMarginRight + itemsGap;
  
    let index = Math.floor(this.items.scrollLeft / itemFullWidth);
    index = Math.max(0, Math.min(index, this.items.children.length - 1)); // Clamp index within valid range
  
    return this.items.children[index] || null;
  }
...
#handlePrevNavigationAction(e) {
    ...
    if (previousItem) {
      ...
      const offset = previousItem.offsetLeft;
      console.log('Scrolling to offset:', offset);

      this.smoothScrollItems(offset)
        .finally(() => {
          this.#isNavigating = false;
  ...
  #handleNextNavigationAction(e) {
    ...
    if (nextItem) {
      ...
      const offset = nextItem.offsetLeft;
      console.log('Scrolling to offset:', offset);
...
#moveItems(position, behavior = 'instant') {
    ...
    if (itemsScrollX < 0) {
      itemsScrollX = 0;
    } else if (itemsScrollX > this.scrollableElement.scrollWidth - this.scrollableElement.offsetWidth) {
      itemsScrollX = this.scrollableElement.scrollWidth - this.scrollableElement.offsetWidth;
    }
  ...
  smoothScrollItems(position) {
      return new Promise((resolve, reject) => {
        ...
        const check = () => {
          const newPos = this.scrollableElement.scrollLeft;
        ...
        console.log('Executing scrollTo:', position);
        this.scrollableElement.scrollTo({
          left: position,
          behavior: 'smooth'
        });
        console.log('Current scrollLeft after scrollTo:', this.scrollableElement.scrollLeft);
